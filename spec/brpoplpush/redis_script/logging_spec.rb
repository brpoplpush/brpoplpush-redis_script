...
RSpec.describe Brpoplpush::RedisScript::Logging do
  before do
  ...
  describe "#logger" do
    ...
    it { is_expected.to eq(Brpoplpush::RedisScript.logger) }
  end
  ...
  shared_examples "delegates to logger" do |level:|
    ...
    it "delegates to logger" do
      expect(logging.send("log_#{level}".to_sym, message)).to eq(nil)
